...
async fn kill_tree_unix(pid: u32, grace_ms: u64) -> Result<bool, String> {
    ...
    // Do not use process-group kill here (`kill -PID`), because if group
    // membership is unexpected it can signal unrelated runner/session
    // processes. Kill the process and its direct children explicitly.
